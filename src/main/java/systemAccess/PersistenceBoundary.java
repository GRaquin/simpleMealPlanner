...
public class PersistenceBoundary implements Serializable {
  ...
  private static final String MEAL_ARRAY_LOCATION = "C:\\Users\\zorac\\Documents\\Meal Planner\\MealSaves.ser"; //Location saved as a static for easier management.
   ...
   */
  public void load(MealListHelper mealListHelper) {
    File savedMeals = new File(MEAL_ARRAY_LOCATION); // This is to find the file and checks if it exists.
    if (savedMeals.exists() && !savedMeals.isDirectory()) {
      try {
        FileInputStream fileStream = new FileInputStream(MEAL_ARRAY_LOCATION); //This line actually gets the data out of the file. Hence why it needs a path.
        ...
        mealListHelper.loadMealList(loadTemp);
  ...
  public void save(MealListHelper mealListHelper) {
    ArrayList<Meal> mealList = mealListHelper.getMealList();
    try {
      FileOutputStream fs = new FileOutputStream(MEAL_ARRAY_LOCATION);
